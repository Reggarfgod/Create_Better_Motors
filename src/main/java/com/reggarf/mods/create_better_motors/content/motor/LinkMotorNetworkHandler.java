...
public class LinkMotorNetworkHandler {
	...
	public void addToNetwork(LevelAccessor world, LinkMotorBehaviour actor) {
		...
		if (actor.blockEntity instanceof LinkMotorTileEntity voidMotor) voidMotor.onConnectToVoidNetwork();
	}

	public void removeFromNetwork(LevelAccessor world, LinkMotorBehaviour actor) {
		if (actor.blockEntity instanceof LinkMotorTileEntity voidMotor) voidMotor.onDisconnectFromVoidNetwork();
	...
	public static class NetworkKey {
		...
		public static NetworkKey fromString(String json) {
			...
			try {
				...
				Create_better_motors.LOGGER.error("Tried to load invalid NetworkKey '" + json + "'");
				return null;
			}
